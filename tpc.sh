...
check_variables()
{
	...
	local count=$(grep "VALIDATE_ANSWER_SETS" $MYVAR | wc -l)
	if [ "$count" -eq "0" ]; then
		echo "VALIDATE_ANSWER_SETS=\"false\"" >> $MYVAR
		new_variable=$(($new_variable + 1))
	fi
